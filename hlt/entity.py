...
class Entity:
    ...
    def radians_between(self, target):
...
class Ship(Entity):
    ...
    def thrust(self, magnitude, angle):
        ...
        :param float angle: The angle to move the ship in (DEGREES)
        ...
        #     return "t %d %d %d"%(self.id, magnitude, angle)
    ...
    def navigate(self,target, game_map, speed, avoid_obstacles=True, max_corrections=90, angular_step=1,
        ...
        params = self.navigate_calc(target, game_map, speed, avoid_obstacles, max_corrections, angular_step,
                 ignore_ships, ignore_planets)
        if params is None:
            return None
        speed, angle = params
        return self.thrust(speed, angle)

    def navigate_calc(self, target, game_map, speed, avoid_obstacles=True, max_corrections=90, angular_step=1,
                 ignore_ships=False, ignore_planets=False):
        """
        ...
        if avoid_obstacles and game_map.obstacles_between(self, target, ignore):
            ...
            return self.navigate_calc(new_target, game_map, speed, True, max_corrections - 1, angular_step)
...
class MovedShip(Entity):
    def __init__(self, ship, mag, angle):
        """
        :param Ship ship:
        :param mag:
        :param angle:
